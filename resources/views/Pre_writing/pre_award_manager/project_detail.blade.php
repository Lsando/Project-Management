                        ...
                        <div class="listing-item listing-item-with-icon">
                            ...
                            <div class="list-group list-group-inline margin-bottom-0">
                                ...
                                <div class="list-group-item col-md-4 col-sm-4">
                                    ...
                                    <span class="text-muted"><a href="{{ asset('docs/' . $project->p_support_document) }}" class="" target="_blank"><span class="icon-download text">document</span></a></span>
                                    @else    
            ...
            <div class="block-divider-text"><span class="fa fa-bookmark-o"></span></div>
                <div class="block-content">
                    @if ( $project->p_state == "Em curso" && $project->psm_id > 1)
                                <div class="app-heading">
                                    <div class="title">
                                        <h3> <span class="icon-thumbs-up"> {{ translate('proposta_aprovada') }} </span> </h3>
                                    </div>
                                </div>
                                <div class="col">
                                    <div class="col-md-2 ">
                            ...
                            {{-- {{dd($nr_days[0]->nr_dias)}} --}}
                                    <div class="block-content">
                                        {{-- <h1>Teste</h1> --}}
                                        <form action='{{route('pre_writing.first_project_approval')}}' method="POST" class="form-horizontal">
                                            @csrf
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-8">
                                                    <select class="form-control" name="p_state" id="" {{($project->p_state == "Rejeitado"||$project->p_state == "Aprovado" )? 'disabled':''}}>
                                                        ...
                                                        <option value="Rejeitado" {{$project->p_state == "Rejeitado"? 'selected':''}}>{{translate('reprovar')}}</option>
                                                    </select>
                                                </div>
                                                @error('p_state')
                                ...
                                {{-- @elseif($nr_days[0]->nr_dias < 30)
                                    <div class="block-content">
                                        <div class="app-heading">
                                            <p>{{translate('projecto_enviado_para_aprovacao_').' '.$nr_days[0]->nr_dias.' '.translate('dias_atras')}}</p>
                                        </div>
                                    </div> --}}
                                @elseif($project->p_state == 'Perdida')
                                <div class="block-content">
                                    <div class="app-heading">
                                        <p>{{translate('projecto_enviado_para_aprovacao_').' '.$nr_days[0]->nr_dias.' '.translate('dias_atras')}}</p>
                                    </div>
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <img src="{{asset('img/ilustrator/rejected.png')}}" style="width:50px;height:50px;" class="img-responsive center-block d-block mx-auto" alt="Sample Image">
                                            <p class="text-center"><strong> {{translate('proposta').' '. translate('perdida')}}</strong></p>
                                        </div>                                        
                                    </div>
                                </div>
                                @endif
